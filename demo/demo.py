...
def do_test(args, cfg, model):
    ...
    shape = util.imread(list_of_ims[0]).shape
    ...
    for path in tqdm(list_of_ims):
        ...
        if len(meshes) > 0:
            ...
            #outImg = np.concatenate((im_drawn_rgb, im_topdown), axis=1)
            #print(outImg.shape)
            outImg = np.zeros((im.shape[0] * 2, im.shape[1], 3), dtype=np.uint8)
            ...
            outImg[:im.shape[0], :, :] = im_drawn_rgb
            left = im.shape[0] // 2 if im.shape[0] % 2 == 0 else im.shape[0] // 2 + 1
            right = im.shape[0] // 2

            outImg[im.shape[0]:, im.shape[1] // 2 - left : im.shape[1] // 2 + right, :] = im_topdown
            util.imwrite(outImg, os.path.join(output_dir, im_name+'.jpg'))
